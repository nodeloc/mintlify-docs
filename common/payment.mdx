...
<Warning>
**转出方固定为申请该 Payment 应用的用户**，无法转出其他人的积分。
请确保申请人账户中有足够的积分（含手续费）。
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Q: 转账可以转给任意用户吗？">
    可以，只要你提供的 `to_user_id` 和 `to_username` 同时正确并指向同一个 NodeLoc 用户。
    转出方固定是申请该 Payment 应用的用户，无法用此接口转出其他人的积分。
  </Accordion>

  <Accordion title="Q: 转账手续费是多少？">
    手续费按申请人当前的信任等级（TL0~TL4）计算，对应站点设置 `points_transfer_fee_rate_tl0` ~ `points_transfer_fee_rate_tl4`。
    申请人扣减 `amount + fee`，接收方到账 `amount`。
  </Accordion>
</AccordionGroup>
